...
let G_TEXTURES = {}; // 【已修复】新增一個全局變量來存放加載好的紋理
...
async function main() {
    ...
    try {
        ...
        G_TEXTURES = textures; // 【已修复】把紋理存儲到全局變量
...
async function handleSpin() {
    ...
    // (前端) 向您的後端服務器發送請求
    try {
        // 【已修复】刪除了 'http://127.0.0.1:5000'，使其變為相對路徑
...
async function playExpandingWildAnimation(reelIndex) {
    ...
    // 【示例】使用 G_TEXTURES 顯示神龍
    const wildTexture = G_TEXTURES[ASSETS.sym_wild];
    if (wildTexture) {
        const dragon = new PIXI.Sprite(wildTexture);
        dragon.anchor.set(0.5);
        dragon.x = reelContainer.x + (reelIndex * 150) + 75; // 示例座標
        dragon.y = reelContainer.y + 200;
        dragon.scale.set(1.5); // 放大
...
function startFakeJackpotTimer() {
    ...
    setTimeout(async () => {
        try {
            // 【已修复】刪除了 'http://127.0.0.1:5000'，使其變為相對路徑
            const response = await fetch('/api/getFakeWinner');
...
async function playBigWinAnimation(level, amount) {
    ...
    // --- 【已修复】新增: 真正顯示金幣 (簡易版) ---
    const coinTexture = G_TEXTURES[ASSETS.sym_dollar];
    if (coinTexture) {
        const coin = new PIXI.Sprite(coinTexture);
        coin.anchor.set(0.5);
        coin.x = GAME_WIDTH / 2;
        coin.y = GAME_HEIGHT / 2 + 100; // 在金額下方
        coin.scale.set(0.5);
    ...
    if (level === 'MEGA') {
        ...
        // (同理，使用 G_TEXTURES[ASSETS.sym_wild] 來創建龍)
    }
    if (level === 'EPIC') {
        console.log("播放 6128.jpg 金龍 + 4720.png '聯' + 6120.jpg 元寶 全屏特效...");
        // (同理，使用 G_TEXTURES[...] 來創建)
    }
